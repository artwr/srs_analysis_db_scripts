USE [SRSAnalysis]
GO
/****** Object:  StoredProcedure [dbo].[Created3DCTables]    Script Date: 05/16/2014 16:40:22 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Created3DCTables]') AND type in (N'P', N'PC'))
BEGIN
...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[Created3DCTables] 
...
BEGIN
	...
	*/
	...
	if exists (
		select * from sysobjects 
			where id = OBJECT_ID(N''allsamples_d3'')
				and OBJECTPROPERTY(id,N''IsUserTable'') = 1
	)
		drop table [SRSAnalysis].[dbo].[allsamples_d3]
